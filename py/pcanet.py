...
class PcaNet:
    ...
    def save(self, path_model,target_name=None):
        # create paths
        if not os.path.exists(path_model):
            os.mkdir(path_model)
        path_filters=path_model+'/filters/'
        path_param=path_model+'/parameters/'
        if not os.path.exists(path_filters):
            os.mkdir(path_filters)
        if not os.path.exists(path_param):
            os.mkdir(path_param)

        # save filters
        with open(path_filters+'filters_l1.obj', 'wb') as file:
            pickle.dump(self.layers['layer_1'].filters, file)
        with open(path_filters+'filters_l2.obj', 'wb') as file:
            pickle.dump(self.layers['layer_2'].filters, file)

        # save parameters
        dict_summary={}
        dict_summary['target_name']=str(target_name)
        dict_summary['model_name']='PcaNet'
        dict_summary['layer_1']={'n_filters': self.layers['layer_1'].n_filters, 'shape': self.layers['layer_1'].filter_shape}
        dict_summary['layer_2']={'n_filters': self.layers['layer_2'].n_filters, 'shape': self.layers['layer_2'].filter_shape}
        dict_summary['shape_in_blocks']=self.shape_in_blocks
        with open(path_param+'summary.json', 'w') as outfile:
    ...
    def load(self, path_model):
        ...
        # load parameters
        with open(path_param+'summary.json', 'r') as outfile:
            dict_summary=json.load(outfile)

        # initialize PcaNet model
        layer_1 = Layer_PcaNet(filters=filters_l1)
        layer_2 = Layer_PcaNet(filters=filters_l2)
        self.__init__(layers=[layer_1, layer_2], shape_in_blocks = tuple(dict_summary['shape_in_blocks']))
    ...
    def inference(self, inputs, verbose=0):
    ...
    def binary_hashing_step(self, inputs, sparsity, verbose=0):
    ...
    def histogram_encoding_step(self, inputs, verbose=0):
    ...
    def predict(self, inputs, sparsity=0.5):

        # inference through PcaNet model
        outputs = self.inference(inputs)

        # hashing step
        hashed_outputs = self.binary_hashing_step(outputs, sparsity=sparsity)

        # histogram block step 
        feature_map = self.histogram_encoding_step(hashed_outputs)
